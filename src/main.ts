...
if (urlParams.get('settings') === '1') {
  ...
  app
    ...
    .then(() => {
      ...
      // Mount CopilotKit React island after app is fully initialized
      import('./copilot/copilot-island').then(({ mountCopilotIsland }) => {
        mountCopilotIsland();
      }).catch((err) => console.warn('[CopilotKit] Failed to mount:', err));
    })
    .catch(console.error);
}
