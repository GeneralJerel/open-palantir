...
import { appBridge } from '@/copilot/app-bridge';
...
export class App {
  ...
  public async init(): Promise<void> {
    ...
    // ── CopilotKit bridge: push initial state + listen for copilot actions ──
    this.syncCopilotBridge();
    this.setupCopilotEventListeners();
  }

  /** Push current app state into the CopilotKit bridge. */
  private syncCopilotBridge(): void {
    const activeLayers = Object.entries(this.state.mapLayers)
      .filter(([, v]) => v === true)
      .map(([k]) => k);

    appBridge.update({
      activePanels: Object.keys(this.state.panels),
  ...
  /** Listen for CustomEvents dispatched by CopilotKit frontend actions. */
  private setupCopilotEventListeners(): void {
    // Navigate to country
